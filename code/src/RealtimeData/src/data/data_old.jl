#=function load_eg_wages(vint::String, reference_forecast::Symbol,
                       year::Int, quarter::Int, directory::String; verbose::Symbol = :low)
    ...
    # Read file
    datadir = directory
    filename = joinpath(datadir, "data", "edge_gurkaynak", string(reference_forecast), "sw" * ref_date * "labor.txt")
    ...
    for line in lines
        ...
        if length(split_line) == 5
            ...
            obs_date    = Dates.lastdayofquarter(Date(parse(Int, obs_year), 3*parse(Int, obs_quarter)))
...
function load_nonfred_data(vint::String, year::Int, quarter::Int;
    ...
    filepath = joinpath(directory, "data", "dlx.csv")
...
function load_realtime_levels(m::AbstractModel, reference_forecast::Symbol,
    ...
    for (mnemonic, vintage) in zip([:JCXFE, :GDI], [Date(1999, 7, 29), Date(2012, 12, 20)])
...
function load_realtime_population_growth(vint::String,
    ...
    data_savepath = joinpath(directory, "raw", "population_data_levels_$vint.csv")
    forecast_savepath = joinpath(directory, "raw", "population_forecast_$vint.csv")
    ...
    if try_disk
        try
            ...
            T0 = findfirst(x -> x == start_date, data[:date])
            data = data[T0:end, :]
...
function transform_realtime_data(m::AbstractModel, levels::DataFrame, population_growth::DataFrame,
    ...
    # We usually use the monthly core PCE index (PCEPILFE) so the model object expects that
    # mnemonic. However, the quarterly core PCE index (JCXFE) has vintages going back further,
    # so we load that in the realtime project instead.
...
function load_rate_expectations_data(m::AbstractModel, df::DataFrame, settings::Dict{Symbol, Any};
    ...
    elseif settings[:rate_expectations_source] == :ois
        ...
        filepath = joinpath(directory, ois_file)
        ...
        for date in DSGE.quarter_range(date_zlb_start(m), date_mainsample_end(m))
            ...
            df_date_ind = findfirst(df[:date], date)
            ref_date_ind = findfirst(ref[:date], date)
            if ref_date_ind > 0
                for h = 1:6
...
function load_semiconditional_data(m::AbstractModel, year::Int, quarter::Int;
    ...
    filepath = joinpath(directory, "longrate.csv")
    if DSGE.VERBOSITY[verbose] >= DSGE.VERBOSITY[:low]
...
function load_semiconditional_data(m::AbstractModel, year::Int, quarter::Int, directory::String;
                                   verbose::Symbol = :none)
    cond_date = DSGE.quartertodate(year, quarter)
    semicond_series = [:DFF, :BAA, :AAA, :BAMLC8A0C15PYEY, :GS10, :GS20, :GS30, :THREEFYTP10, :FYCCZA]
    ...
    # Load long rate from CSV
    filepath = joinpath(directory, "input_data", "data", "longrate.csv")
    if DSGE.VERBOSITY[verbose] >= DSGE.VERBOSITY[:low]
        println(" * Reading semiconditional FYCCZA from $filepath...")
    end
    series = CSV.read(filepath)
        ...
        if cond_type == :full
            ...
            if settings[:use_staff_forecasts]
                ...
                if size(ref, 1) == 0
                    ...
                    ind  = findfirst(ref[:date], DSGE.quartertodate(year, quarter))
...
function load_realtime_data(m::AbstractModel, reference_forecast::Symbol,
                            ...
                            verbose::Symbol = :low, directory::String = dataroot(m))
    ...
    savepath = joinpath(directory, "realtime" * addl * ".csv")
    ...
    if reload_data
        ...
        population_growth, _ = load_realtime_population_growth(vint, y, q, settings, directory, verbose = verbose)
    ...
    DSGE.nan_cond_vars!(m, df; cond_type = cond_type)
